...
class YelpEnsembleModel:
    def __init__(self,
        ...
        self.random_forest_base_path = random_forest_base_path
    ...
    def get_data_DataFrames(self,
                            user_names_dict: dict) -> (dict, dict, dict):
        """
        This function encapsulates the function calls necessary to read in the data for each user, and each model that
        we want ensemble predictions for
        ...
        user_cluster_preds = self.read_in_data(base_path=self.user_cluster_base_path, user_names=user_names_dict)

        bus_cluster_preds = self.read_in_data(base_path=self.bus_cluster_base_path, user_names=user_names_dict)

        rf_preds = self.read_in_data(base_path=self.random_forest_base_path, user_names=user_names_dict)
    ...
    def get_data_for_user(self,
        ...
        from each model. It outputs one standardized pandas Data Frame containing predictions for each model in
        the ensemble.
        ...
        user_df = self.standardize_df_names(dfx=user_dat, base_set='user')

        bus_dat = bus_cluster_preds[user]
        bus_df = self.standardize_df_names(dfx=bus_dat, base_set='bus')

        rf_df = None
        try:
            rf_dat = rf_preds[user]
            rf_df = self.standardize_df_names(dfx=rf_dat, base_set='rf')
        except:
            rf_df = None
    ...
    def get_ensemble_rank_for_business_id(self,
                                          bus_id: str,
                                          user_df: pd.DataFrame,
                                          bus_df: pd.DataFrame,
                                          rf_df: pd.DataFrame,
                                          ensemble_method: str = "best-average") -> int:
        """
        ...
        options: "best-average", and "best-vote"
        :return: (int) The ensemble derived ranking for this specific business
        """
        user_rank = self.get_df_rank(user_df, bus_id)
        bus_rank = self.get_df_rank(bus_df, bus_id)

        total_rank = user_rank + bus_rank

        if rf_df is not None:
            rf_rank = self.get_df_rank(rf_df, bus_id)
        ...
        elif ensemble_method != "best-average":
    ...
    def ensemble_ordered_user_resturant_ranking(self,
                                                user_df: pd.DataFrame,
                                                bus_df: pd.DataFrame,
                                                rf_df: pd.DataFrame,
                                                ensemble_method: str = "best-average") -> pd.DataFrame:
        """
        This method combines the predictions of all underlying models in the ensemble to generate a final prediction
        for a specific user
        ...
        for bus_id in business_ids:
            ensemble_rank = self.get_ensemble_rank_for_business_id(bus_id, user_df, bus_df, rf_df, ensemble_method)
    ...
    def ensemble_model_results_and_display_top_x_resturants(self,
                                                            user_names_dict: dict,
                                                            top_x: int = 5,
                                                            ensemble_method: str = "best-average") -> None:
        """
        ...
        The two options are: best-vote, best-average
        :return: None
        """
        user_cluster_preds, bus_cluster_preds, rf_preds = self.get_data_DataFrames(user_names_dict=user_names_dict)
        for k in user_names_dict.values():
            user_df, bus_df, rf_df = self.get_data_for_user(user=k,
...
def main(top_x_restaurants: int = 10,
         ensemble_methods_to_use: list = ['best-vote', 'best-average'],
         user_cluster_base_path: str = "/home/taylor/Duke/ECE 682/ECE_682_Final_Project/Analysis/data/user_cluster_rankings/user_cluster_rankings/",
         business_cluster_base_path: str = "/home/taylor/Duke/ECE 682/ECE_682_Final_Project/Analysis/data/final_user_recs_BC/final_user_recs/",
         random_forest_base_path: str = "/home/taylor/Duke/ECE 682/ECE_682_Final_Project/Analysis/data/RF-closest-rankings/"
         ) -> None:
    ...
    best-average: produces the overall ensemble ranking by averaging the ranking for each specific business across
    ...
    :param ensemble_methods_to_use: (list) a list of the different ensemble methods to try.
    The complete set of options are: 'best-vote', and 'best-average'
    :param user_cluster_base_path: (str) a full path to the directory containing the predictions for
    the user-cluster model
    :param business_cluster_base_path: (str) a full path to the directory containing the predictions for
    the business-cluster model
    ...
    :return: None
    """
    yem = YelpEnsembleModel(user_cluster_base_path=user_cluster_base_path,
                            business_cluster_base_path=business_cluster_base_path,
                            random_forest_base_path=random_forest_base_path)
    user_names_dict = get_test_user_names_dict()

    for method in ensemble_methods_to_use:
        print(f'+===============================================+')
        print(f'|   Top {top_x_restaurants} restaurants using {method} method    ')
        print(f'|')
        yem.ensemble_model_results_and_display_top_x_resturants(user_names_dict=user_names_dict,
